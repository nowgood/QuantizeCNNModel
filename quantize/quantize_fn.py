...
class QuantizeWeightOrActivation(object):
    def __init__(self):
        ...
        self.quantize_fn = Quantize.apply  # 量化函数

    def _quantize_weights_bias(self, weight):
    ...
    def quantize(self, m):
        ...
        if type(m) == nn.Conv2d or type(m) == nn.Linear:
            ...
            quantize_w = self._quantize_weights_bias(m.weight)  # 第2步, 量化权重
        ...
        if type(m) == nn.Linear:  # 量化 bias
            ...
            quantize_b = self._quantize_weights_bias(m.bias)
